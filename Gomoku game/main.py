...
def redrawBoard(window):
    ...
    for i in range(gameRows):
        # draw the line (window , typeOftheLine , (start positon),(end position),thickness)
        pygame.draw.line(window,blackLine,(cellCenter,i*cellSize+cellCenter),(width-cellCenter,i*cellSize+cellCenter),1) #start position(the first point in the x axis,current point in y axis ) ---> end position(the last point in the x axis , current point in y axis)
    for j in range (gameCols):
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if buttonX <= x < buttonX + limitX and buttonY <= y < buttonY + LimitY:
                    ...
                    if 5 <= gameCols <= 19 and 5 <= gameRows < 19:
                        resize(gameWindow, 1) #change size of the window +1
                        turn=1
                        board.clear()
                        generateBoard(board)
